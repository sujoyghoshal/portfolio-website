...
const techCategories = [
  {
    label: 'Languages', color: '#F59E0B', bg: 'rgba(245,158,11,0.12)',
    items: [
  ...
  {
    label: 'Frontend', color: '#00D4FF', bg: 'rgba(0,212,255,0.12)',
    items: [
  ...
  {
    label: 'Backend', color: '#10B981', bg: 'rgba(16,185,129,0.12)',
    items: [
  ...
  {
    label: 'Database', color: '#8B5CF6', bg: 'rgba(139,92,246,0.12)',
    items: [
  ...
  {
    label: 'DevOps & Cloud', color: '#EC4899', bg: 'rgba(236,72,153,0.12)',
    items: [
...
const extraTags = [
  'Microservices', 'WebSockets', 'JWT Auth', 'REST / JSON',
  'Maven', 'Gradle', 'JUnit', 'Swagger', 'Vite', 'Jest',
      ...
      <div className="section-center">
        ...
        >
          ...
          <p className="section-copy">
            Product-minded engineering, clean interfaces, and scalable backend thinking — all in one focused profile.
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          ...
          >
            <div>
              <span className="about-badge">Profile</span>
              <h3 className="text-xl font-extrabold text-[var(--text)] mt-3 mb-2">
                Software Engineer @{' '}
                <span className="gradient-text">Capgemini</span>
              </h3>
            ...
            <div className="flex flex-col gap-2">
              {infoItems.map(({ Icon, value, href }) => (
                <motion.div key={value} whileHover={{ x: 4 }} className="info-row group">
                  <Icon size={14} className="text-[#00D4FF] shrink-0" />
                  {href
                    ? <a href={href} target="_blank" rel="noreferrer"
                        className="text-sm truncate hover:text-[#00D4FF] transition-colors">
                        {value}
                      </a>
                    : <span className="text-sm truncate">{value}</span>
                  }
                  {href && <ExternalLink size={10} className="ml-auto shrink-0 opacity-0 group-hover:opacity-60 transition-opacity" />}
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          >
            <span className="about-badge mb-3">Proficiency</span>
            <h3 className="text-xl font-extrabold text-[var(--text)] mb-5">Core Strengths</h3>

            <div className="flex flex-col flex-1">
              {progressItems.map((s, i) => (
                <SkillBar key={s.name} name={s.name} pct={s.pct} color={s.color} delay={i * 0.07} />
              ))}
            </div>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <span className="about-badge">Tech Stack</span>
              <h3 className="text-xl font-extrabold text-[var(--text)] mt-3">
                Skills &amp; Technologies
              </h3>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {techCategories.map(({ label, color, bg, items }, ci) => (
              <motion.div
                key={label}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {items.map(({ name, Icon, color: ic }) => (
                    <motion.span
                      ...
                      style={{ '--pill-color': ic }}
                    >
                      <Icon size={11} style={{ color: ic, flexShrink: 0 }} />
                      {name}
                    </motion.span>
                  ))}
                </div>
